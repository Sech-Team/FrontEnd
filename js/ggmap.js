...
function updateMapAndDistance() {
  let longitude = document.getElementById('value5').value;
  let latitude = document.getElementById('value6').value;
  if (longitude !== '' && latitude !== '') {
      // move the map to the location
      setupMap([parseFloat(longitude), parseFloat(latitude)]);
      
      let nearestStation = null;
      let minDistance = Infinity;

      mrtStations.forEach(station => {
          const distance = calculateDistance([parseFloat(longitude), parseFloat(latitude)], station.coordinates);
          if (distance < minDistance) {
          minDistance = distance;
          nearestStation = station;
          }
